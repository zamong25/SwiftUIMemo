...
struct MemoListScene: View {
    @EnvironmentObject var store: MemoStore //scenedelegate에서 memostore 인스턴스를 커스텀 공유데이터로 등록했음, 뷰가 생성되는 시점에 공유데이터 목록을 확인하고 store속성과 동일한 형식을 가진 객체가 등록되어 있다면 여기에 자동으로 저장해줌, 스유에서는 이런방식으로 하나의 데이터를 여러뷰에서 공유함
    ...
    var body: some View {
        ...
        NavigationView { // 다음화면으로 전환해줄 navigation view //화면 상단에 빈 navigationbar가 출력됨
            List {
                ForEach(store.list) { memo in // closure 추가 //배열에 저장되어 있는 메모가 memo(parameter)로 전달됨
                    NavigationLink(destination: DetailScene(memo: memo),
                        label: {
                            MemoCell(memo: memo) //extract serveview로 인해 구조체와 호출하는 코드로 바뀌어짐
                        })
                    }
...
struct MemoListScene_Previews: PreviewProvider {
    static var previews: some View {
        MemoListScene()
        //preview에서 사용할 memostore를 공유데이터로 등록
            .environmentObject(MemoStore())
            .environmentObject(DateFormatter.memoDateFormatter) // 이것도 커스텀 공유데이터로 등록
    }
}
